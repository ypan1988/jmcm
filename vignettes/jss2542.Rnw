...
<<mcd-beta-start-value, eval=FALSE>>=
lm.obj <- lm(Y ~ X - 1)
bta0 <- coef(lm.obj)
@
...
<<mcd-lambda-gamma-start-value, eval=FALSE>>=
resid(lm.obj) -> res
...
<<hpc-beta-start-value, eval=FALSE>>=
lm.obj <- lm(Y ~ X - 1)
bta0 <- coef(lm.obj)
@
...
<<hpc-lambda-gamma-start-value, eval=FALSE>>=
resid(lm.obj) -> res
lmd0 <- coef(lm(log(res ^ 2) ~ Z - 1))
gma0 <- c(pi / 2, rep(0, lgma-1))
@
