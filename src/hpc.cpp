...
namespace jmcm {
    ...
    void HPC::UpdateLambdaGamma(const arma::vec& x)
    {
	set_lmdgma(x);
    }
    ...
    arma::vec
    HPC::Wijk(const int i, const int j, const int k) {
	int n_sub = m_.n_rows;
	int n_gma = W_.n_cols;
    ...
    arma::vec
    HPC::CalcTijkDeriv(const int i, const int j, const int k,
    ...
    arma::mat
    HPC::CalcTransTiDeriv(const int i, const arma::mat &Phii, const arma::mat &Ti)
    {
	int n_gma = W_.n_cols;
	...
	for (int k = 1; k != m_(i); ++k) {
	    for (int j = 0; j <= k; ++j) {
		result.submat(j*n_gma,k,(j*n_gma+n_gma-1),k) = CalcTijkDeriv(i,k,j,Phii,Ti);
	    }
	}
