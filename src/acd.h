...
//
// acd.h: implementation of joint mean-covariance models based on
...
namespace jmcm {
	...
	arma::mat get_W() const { return W_; }
        ...
        int       get_m(const int i) const {
	    return m_(i);
	}
        
	arma::vec get_Y(const int i) const {
	    arma::vec Yi;
	    if (i == 0) Yi = Y_.subvec(0,m_(0)-1);
	    else {
	...
	arma::vec get_gamma() const { return gamma_; }
		...
		if (i == 0) {
		    ...
		    Ti_inv = pan::ltrimat(m_(0), invTelem_.subvec(first_index, last_index), true);
		} 
		else {
		    int first_index = 0;
		    for (int idx = 0; idx != i; ++idx) {
        ...
        arma::mat get_Sigma(const int i) {
	    arma::mat Ti = get_T(i);
            arma::mat Di = get_D(i);
        ...
        arma::mat get_Sigma_inv(const int i) {
	    arma::mat Ti = get_T(i);
	    ...
	    arma::vec mui;
	    if (i == 0) mui = Xbta_.subvec(0,m_(0)-1);
	    else {
		int index = arma::sum(m_.subvec(0,i-1));
	    ...
	    arma::vec ri;
	    if (i == 0) ri = Resid_.subvec(0,m_(0)-1);
	    else {
		int index = arma::sum(m_.subvec(0,i-1));
	...
	void Grad2(arma::vec& grad2);
        ...
        void UpdateACD(const arma::vec& x);
	...
	arma::vec TDResid2_;
	    ...
	    arma::vec TiDiri;
	    if (i == 0) TiDiri = TDResid_.subvec(0,m_(0)-1);
	    else {
		int index = arma::sum(m_.subvec(0,i-1));
	    ...
	    arma::vec TiDiri2;
	    if (i == 0) TiDiri2 = TDResid2_.subvec(0,m_(0)-1);
	    else {
		int index = arma::sum(m_.subvec(0,i-1));
